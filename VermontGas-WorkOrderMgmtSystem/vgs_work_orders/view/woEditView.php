...
if (isset($_SESSION['premises_SR_ST'])
&& $form->isCreateMode()
&& $_GET['AUTOCRT']=='true') {
	$srsts = array_keys($_SESSION['premises_SR_ST']);
	echo '<div style="text-align: left; color:green">' . 
			count($srsts) . ' WOs left to create: ' .
			implode(', ', $srsts) .
			'</div>';
}
...
?>
	...
	<tr>
		...
		&& isset($form->fh->fieldGroups['gl'])) {
			$form->renderFieldGroup('gl');
		}
...
$('document').ready(function(){
	// This handles highlighting of the "Completion" checkbox (COMPLETE_CB). 
	...
	$('#LK_THREAT').change(function(){
		changeDropDownList('LK_THREAT', 'LK_SUB_THREAT', 'LK_SUB_THREAT');
	});

	$('#LK_LEAK_ORIGIN').change(function(){
		changeDropDownList('LK_LEAK_ORIGIN', 'LK_EQUIPTYPE', 'LK_EQUIPMENT_TYPE');
	});

	$('#WO_ESTIMATED_COST').change(function(){
		calcEstCostPerFoot();
	});
	$('#WO_EST_COST_PER_FOOT').change(function(){
		calcTotalEstCost();
	});
	$('#WO_ESTIMATED_LENGTH').change(function(){
		calcTotalEstCost();
	});
	
	$("#OPTIONAL_PIPE_FIELDS").change(function (){
		// Set blank values for pipe info fields when optional checkbox is checked
		if ($(this).is(':checked')) {
			$("#WO_PIPE_MATERIAL").val('N/A');
			$("#WO_PIPE_SIZE").val('0');
			$("#WO_COATING_TYPE").val('N/A');
		}
	});
})
...
function calcTotalManHours() {
	...
	var crewSize = $.trim($('#LK_CREW_SIZE').val());
...
function calcTotalEstCost() {
	var estCostPerFoot = parseFloat($.trim($('#WO_EST_COST_PER_FOOT').val()));
	var estLength = parseFloat($.trim($('#WO_ESTIMATED_LENGTH').val()));

//	alert('estCostPerFoot = ' + estCostPerFoot + '\n estLength = ' + estLength);
	if (estCostPerFoot != 0.0 && !isNaN(estCostPerFoot)
	&&  estLength != 0.0 && !isNaN(estLength)) 
	{
		var estTotalCost = estCostPerFoot * estLength;
		if (isNaN(estTotalCost)) {
			$('#WO_ESTIMATED_COST').val('');
		} else {
			estTotalCost = roundedToFixed(estTotalCost, 2);
			$('#WO_ESTIMATED_COST').val(estTotalCost);
		}
	}
}

function calcEstCostPerFoot() {
	var estTotalCost = parseFloat($.trim($('#WO_ESTIMATED_COST').val()));
	var estLength = parseFloat($.trim($('#WO_ESTIMATED_LENGTH').val()));
//	alert('estTotalCost = ' + estTotalCost + '\n estLength = ' + estLength);

	var estCostPerFoot = 0.0;

	if (estTotalCost != 0.0 && !isNaN(estTotalCost)
	&&  estLength != 0.0 && !isNaN(estLength)) 
	{
		estCostPerFoot = estTotalCost / estLength;
		if (isNaN(estCostPerFoot)) {
			$('#WO_EST_COST_PER_FOOT').val('');
		} else {
			estCostPerFoot = roundedToFixed(estCostPerFoot, 2);
			$('#WO_EST_COST_PER_FOOT').val(estCostPerFoot);
		}
	}
}
...
function changeDropDownList( select1_ID, ddPrefix2, select2_ID ) {
	var select1_val = $('#' + select1_ID).val();
	var ddList = ddPrefix2 + '_' + select1_val;
	//alert('ddList = ' + ddList + '\nselect2_ID = ' + select2_ID); 
	getDropDownList(ddList, select2_ID);
}
